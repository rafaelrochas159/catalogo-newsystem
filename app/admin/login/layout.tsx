...
export default function AdminLayout({
  ...
  const checkAuth = async () => {
    try {
      ...
      if (!session) {
        // só redireciona para /admin/login se não estivermos na página de login
      ...
      // em caso de erro, redireciona apenas se não for a própria tela de login
      if (pathname !== '/admin/login') {
        router.push('/admin/login');
      }
    } finally {
  ...
  // Usuário não autenticado: exibe spinner enquanto redireciona
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center">
